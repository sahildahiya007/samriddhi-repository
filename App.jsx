...
import LuxuryDetailsPage from "./components/LuxuryDetailsPage.jsx";
...
const shouldUseLuxuryDetails = (property) => {
  if (!property) return false;
  const searchable = `${property.title || ""} ${property.details || ""} ${property.cardDescription || ""}`;
  return Boolean(
    property.isLuxury ||
      property.typeText ||
      property.reraNumber ||
      ...
      property.detailsSections?.length ||
      /luxury|elan|sobha|signature residence|penthouse/i.test(searchable),
  );
};
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hover
          ? "0 34px 80px rgba(18,15,12,0.28), 0 10px 24px rgba(217,123,80,0.12)"
          : "0 18px 52px rgba(18,15,12,0.18)",
      }}
    >
      {/* Image */}
      <div className="relative overflow-hidden" style={{ height: "clamp(250px, 27vw, 370px)", borderRadius: "28px 28px 0 0" }}>
        <img
          src={property.image}
          alt={property.title}
          className="w-full h-full object-cover"
          style={{ transition: "transform 0.7s cubic-bezier(0.22,1,0.36,1)", transform: hover ? "scale(1.055)" : "scale(1)" }}
        />
        {/* Bottom gradient */}
        <div
          className="absolute inset-x-0 bottom-0 h-28 pointer-events-none"
          style={{ background: "linear-gradient(to top, rgba(0,0,0,0.68) 0%, rgba(0,0,0,0.24) 58%, transparent 100%)" }}
        />
        {/* Type badge */}
        <span
          className="absolute top-4 left-4 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-[0.16em]"
          style={{
            background: "linear-gradient(135deg, #D97B50 0%, #C96A38 100%)",
            color: "#fff",
            boxShadow: "0 8px 18px rgba(217,123,80,0.34)",
          }}
        >
          {typeLabel}
        </span>
        {/* Price on image */}
        <span
          className="price-text absolute bottom-4 left-5 text-[15px] font-bold"
          style={{ color: "#fff", textShadow: "0 2px 8px rgba(0,0,0,0.70)" }}
        >
          {displayPrice}
        </span>
        {/* Rating on image */}
        {property.rating && (
          <span
            className="absolute bottom-4 right-5 flex items-center gap-1 text-[12px] font-semibold"
            style={{ color: "#fff", textShadow: "0 2px 8px rgba(0,0,0,0.60)" }}
          >
            <Star style={{ width: 13, height: 13, fill: "#FFD580", color: "#FFD580" }} />
            {property.rating}
          </span>
        )}
        {/* Wishlist */}
        <button
          ...
          style={{
            ...
            backgroundColor: isWishlisted ? colors.accent : "rgba(22,19,16,0.38)",
            backdropFilter: "blur(14px)",
            boxShadow: isWishlisted ? "0 0 0 2px rgba(217,123,80,0.35)" : "0 10px 22px rgba(0,0,0,0.22)",
          }}
        >
          <Heart style={{ color: "#fff", fill: isWishlisted ? "#fff" : "none", width: 18, height: 18 }} />
        </button>
      </div>
      ...
      >
        <h2
          ...
          style={{ fontFamily: "'Playfair Display', serif", color: colors.dark, fontSize: 18 }}
        >
          {property.title}
        </h2>
        <p className="flex items-center gap-1.5 mb-3 truncate" style={{ color: colors.body, fontSize: 12 }}>
          ...
          <div className="flex gap-1.5 mb-3 overflow-hidden">
            {(property.amenities || []).slice(0, 2).map((a) => (
              <span
                ...
                style={{ backgroundColor: "#F2E5D4", color: "#5A4638", fontSize: 10 }}
              >
                {a}
              </span>
            ))}
            {(property.amenities || []).length > 2 && (
              <span className="px-2.5 py-1 rounded-full font-medium" style={{ backgroundColor: "#F2E5D4", color: "#5A4638", fontSize: 10 }}>
                +{(property.amenities || []).length - 2}
              </span>
            )}
          </div>
        ...
        <div className="flex gap-3">
          <a
            href={`tel:+${callNumber}`}
            className="flex-1 flex items-center justify-center gap-1.5 rounded-2xl font-semibold"
            style={{
              background: "linear-gradient(135deg, #D97B50 0%, #C06030 100%)",
              color: "#fff",
              height: 42,
              fontSize: 13,
              boxShadow: "0 10px 20px rgba(217,123,80,0.24)",
            }}
            onClick={(e) => e.stopPropagation()}
          >
          ...
          <button
            ...
            style={{
              ...
              color: colors.dark,
              border: "1px solid rgba(151,111,73,0.22)",
              height: 42,
              fontSize: 13,
            }}
            onClick={(e) => {
...
function PropertyCarousel({ properties, onClick, wishlist, onToggleWishlist, loading }) {
  return (
    <div className="relative">
      <div className="grid gap-6 px-4 sm:grid-cols-2 lg:grid-cols-3 md:gap-8 md:px-0">
        {loading
          ? [1,2,3,4].map((n) => (
...
function PropertyGrid({ properties, onClick, wishlist, onToggleWishlist, loading }) {
  ...
  return (
    <section
      id="sale"
      ref={ref}
      className="relative py-14 md:py-20 px-3 md:px-8 overflow-hidden"
      style={{
        backgroundImage: `linear-gradient(138deg, rgba(12,11,10,0.72) 0%, rgba(27,22,18,0.68) 48%, rgba(49,36,26,0.62) 100%), radial-gradient(circle at 20% 8%, rgba(232,149,110,0.18) 0%, rgba(232,149,110,0) 42%), url('${bg.sale}')`,
        backgroundSize: "cover",
    ...
    >
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <span
            className="inline-block px-5 py-2 rounded-full text-[11px] font-bold uppercase tracking-[0.24em] mb-4"
            style={{
          ...
          <p className="mx-auto max-w-2xl text-base md:text-lg leading-relaxed" style={{ color: "#F5E6D3" }}>
            Quietly curated residences with verified details, elevated locations, and an advisory experience built for serious buyers.
          </p>
        </div>
        {sale.length > 0 ? (
          <PropertyCarousel
...
function RentalsShowcase({ properties, onClick, wishlist, onToggleWishlist, loading }) {
  ...
  return (
    <section
      id="rentals"
      ref={ref}
      className="relative px-3 md:px-8 py-14 md:py-20 overflow-hidden"
      style={{
        backgroundImage: `linear-gradient(136deg, rgba(12,12,12,0.74) 0%, rgba(29,27,24,0.70) 44%, rgba(54,38,28,0.66) 100%), radial-gradient(circle at 82% 14%, rgba(232,149,110,0.18) 0%, rgba(232,149,110,0) 38%), url('${bg.rent}')`,
        backgroundSize: "cover",
        backgroundPosition: "center",
        backgroundAttachment: "fixed",
      }}
    >
      <div className="max-w-7xl mx-auto w-full relative z-10">
        ...
        >
          ...
          >
            Move-in-ready homes for flexible city living, selected for finish quality, security, and access to Gurugram's business corridors.
          </p>
        </div>
...
function AppInner() {
  ...
  return (
    ...
    >
      ...
      {isModalOpen && shouldUseLuxuryDetails(selected) ? (
        <LuxuryDetailsPage
          property={selected}
          onClose={closeProperty}
          onCall={(property) => {
            const phone = property.contacts?.sales || "+91 9876543210";
