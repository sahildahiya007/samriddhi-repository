        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          >
            ...
            <span className="hidden md:inline">
              Samriddhi <span style={{ color: colors.accent }}>Properties</span>
            </span>
          </button>
        </div>
...
function AdminPanel({
  ...
  const loadAdmins = async () => {
    ...
    try {
      const list = await requestApi("/api/admins", {
        ...
        >
          <div className="p-6 pb-4">
            <h4
              ...
              style={{ fontFamily: "'Playfair Display', serif", color: "#fff" }}
            >
              Samriddhi <span style={{ color: colors.accent }}>Admin</span>
            </h4>
            <p
  ...
  const submit = async (e) => {
    ...
    try {
      if (mode === "register") {
        ...
        // Unified login — tries admin first, then regular user
        const res = await requestApi("/api/auth/unified-login", {
          method: "POST",
          body: JSON.stringify({ identifier: email, password }),
        });
        if (!res?.token) throw new Error("Login failed");
        if (res.type === "admin" && onAdminAuth) {
          onAdminAuth(res.token, res.user);
        } else {
          onAuth(res.token, res.user);
        }
      }
...
export default function App() {
  ...
  const toggleWishlist = async (propertyId) => {
    ...
    try {
      const endpoint = `/api/wishlist/${propertyId}`;
  ...
  // Load registered users when admin opens analytics
    ...
    >
      ...
      <AdminPanel
        ...
        onAddAdmin={async (data) => {
          return requestWithAuth("/api/admins", {
            method: "POST",
        ...
        <div className="max-w-6xl mx-auto px-6 py-14 md:py-16">
          <div className="grid md:grid-cols-3 gap-10 md:gap-8 mb-10">
            <div>
              ...
              >
                Samriddhi{" "}
                <span style={{ color: colors.accent }}>Properties</span>
              </h4>
              <p
          ...
          >
            <div className="flex flex-col md:flex-row items-center justify-between gap-3">
              <p
                ...
                style={{ color: "rgba(245,230,211,0.35)" }}
              >
                © 2026 Samriddhi Properties. All rights reserved.
              </p>
